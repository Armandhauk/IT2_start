...
kelvin=273.15
hoved = input("Hva vil du se på? lat, lon, timezone, timezone_offset, daily  ")
hoved = hoved.lower()
if hoved != "daily":
    print(vaerdata[hoved])
 
elif hoved =="daily":
    dag = int(input("Hvilken dag vil du se på? (1-8) "))
    info = input("Hva vil du se på fra denne dagen? \ndt, sunrise, sunset, moonrise, moonset, moonphase, temp, feels_like, pressure, humidity, dew_point, \nwind_speed, wind_deg, wind_gust, weather, clouds, pop, uvi  ")
   
    if info != "temp" and info != "feels_like" and info != "weather":
        print(vaerdata["daily"][dag-1][info])
 
    elif info == "temp":
        temp_detalj = input("Hvilken temperatur vil du se? day, min, max, night, eve, morn ")
        print(f"{float((vaerdata["daily"][dag-1][info][temp_detalj]))-kelvin:.2f} grader celcius")
        
    elif info == "feels_like":
        feel_detalj = input("Hvilken temperatur vil du se at det føles ut som? day, night, eve, morn ")
        print(vaerdata["daily"][dag-1][info][feel_detalj])
 
    elif info == "weather":
        vaer_detalj = input("Hva om været vil du se? id, main, description, icon ")
        print(vaerdata["daily"][dag-1][info][0][vaer_detalj])
